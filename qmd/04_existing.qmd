...
This section presents a description and preliminary analysis of the study area. This includes a physical description of the site, a discussion of existing traffic volumes, and a base \acr{LOS} analysis. The current traffic safety of the study area is also discussed.
...
@eq-intersection-crash-rate gives the crash rate of an intersection, and @eq-segment-crash-rate gives the crash rate of a segment, where `AADT` is Annual Average Daily Traffic (the number of vehicles entering the intersection or segment), `MEV` is million entering vehicles, and `MVMT` is million vehicle-miles traveled. The AADT was provided by \acr{UDOT} for this stretch of University Ave., and is given as 30550 vehicles per day.

$$
Crash\ Rate_{Intersection} = \frac{1000000 \times crashes}{365 \times years \times AADT}\ \text{crashes per MEV}
$$ {#eq-intersection-crash-rate}

$$
Crash\ Rate_{Segment} = \frac{1000000 \times crashes}{365 \times years \times AADT \times length}\; \text{crashes per MVMT}
