...
#| tbl-cap: "Trips After Pass-by Reduction"

tar_read(tripgen) %>% 
  select(desc, code, mu_enter, mu_exit, mu_tot) %>% 
  mutate(pb_rate = c(0.43, 0, 0.26),
         pb_enter = round(mu_enter*(1-pb_rate)),
         pb_exit = round(mu_exit*(1-pb_rate)), 
         pb_tot = pb_enter + pb_exit
  ) %>% 
  select(-mu_enter, -mu_exit, -mu_tot) %>% 
  flextable() %>% 
