...
@fig-trip-distribution shows the distribution of site trips by entry/exit direction, but these trips need to be assigned to specific intersection movements. This depends substantially on the access layout of the site. There are two accesses to the site: one on 1200 South and one on University Ave.^[This access is shared with the development to the south.]---for a full discussion of access and site layout see @sec-site-layout.
...
- All pass-by trips are assumed to use the University Ave. driveway for both entering and exiting (this includes left turns onto University Ave. exiting the site).
...
Tables [-@tbl-base-tripgen]--[-@tbl-tripgen-passby] provide information on the total number of site trips generated (both primary and pass-by). Using this information and the above assumptions, the site trips are assigned to the intersection movements as shown in @fig-site-assignment.

```{r}
#| label: fig-site-assignment
#| fig-cap: Site trips assigned to intersection movements.

knitr::include_graphics("../images/output/site_trips.png")
```
