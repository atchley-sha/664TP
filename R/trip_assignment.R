...
assign_trips <- function(access, distribution, UA_Lpct, TCD_Tpct, site_trips){
  ...
  # In order to figure out the # of trips on the remaining movements, we have to hard-code what each movement is a sum of. It's done iteratively to make reading a bit easier since the names are already cryptic and some movements are sums of movements that are themselves sums.
  ...
  sum_trips1 <- tribble(
    ...
    add_pb_trips <- full_join(prelim_pcts, sum_trips, by = "NAME") %>% 
